<template>
  <div class="user-list-wrapper">
    ...
    <div class="results">
      <UserListAdmUserCard
        v-for="user in this.usersDisplay"
        :key="user.id"
        v-bind:user="user"
    ...
    <Pagination2
      v-bind:totalItems="this.usersFiltered.length"
...
export default {
  ...
  methods: {
      ...
      if (filter.name) {
        this.usersFiltered = this.usersFiltered.filter(
          (user) =>
            ...
            user.username.toLowerCase().includes(filter.name)
        );
      }

      if (filter.type != "Todos") {
        this.usersFiltered = this.usersFiltered.filter(
          (user) => user.type == filter.type
        );
      }

      if (filter.state != "Todos") {
        this.usersFiltered = this.usersFiltered.filter(
          (user) => user.state == filter.state
        );
      }
    ...
    changePage(page) {
      ...
      this.usersDisplay = this.usersFiltered.slice(
